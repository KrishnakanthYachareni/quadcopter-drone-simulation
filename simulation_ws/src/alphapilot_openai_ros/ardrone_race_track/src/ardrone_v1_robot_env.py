"""ARDrone robot environment definition.

This module is based on the OpenAI ROS ParrotDrone robot environment originally developed by
Miguel Angel Rodriguez, The Construct.

url: https://github.com/edowson/openai_ros/blob/kinetic-devel/openai_ros/src/openai_ros/robot_envs/parrotdrone_env.py
"""
...
class ARDroneEnv(robot_gazebo_env.RobotGazeboEnv):
    """Superclass for all CubeSingleDisk environments.
    """

    def __init__(self):
        """
        Initializes a new ARDroneEnv environment.
        ...
        * /drone/land: Publish to make ARDrone Land

        Args:
        """
        rospy.logdebug("Start ARDroneEnv INIT...")
        ...
        super(ARDroneEnv, self).__init__(controllers_list=self.controllers_list,
        ...
        rospy.Subscriber("/drone/contacts", ContactsState, self._contacts_callback)
        ...
        rospy.logdebug("Finished ARDroneEnv INIT...")
    ...
    def _check_contacts_ready(self):
        self.contacts = None
        rospy.logdebug("Waiting for /drone/contacts to be READY...")
        while self.contacts is None and not rospy.is_shutdown():
            try:
                self.contacts = rospy.wait_for_message("/drone/contacts", ContactsState, timeout=5.0)
                rospy.logdebug("Current /drone/contacts READY=>")

            except:
                rospy.logerr("Current /drone/contacts not ready yet, retrying for getting contacts state")
    ...
    def _contacts_callback(self, data):
        self.contacts = data
    ...
    def wait_time_for_execute_movement(self):
        """
        Because this AR Drone position is global, we really dont have
